...
const CorpusProjectionChart = ({ data }) => {
    ...
    return (
        ...
        >
            <h3 className="engine-chart-title">Corpus Depletion Trajectory</h3>
            <ResponsiveContainer width="100%" height={300}>
                <AreaChart data={data} margin={{ top: 5, right: 20, bottom: 5, left: 10 }}>
                    ...
                    <Tooltip contentStyle={{ background: 'rgba(11,15,28,0.95)', border: '1px solid #1F2937', borderRadius: 8, fontSize: '0.7rem' }}
                        formatter={(val, name) => [formatINR(val), name === 'corpus' ? 'Corpus' : 'Withdrawal']} />
                    <ReferenceLine y={0} stroke="#EF4444" strokeDasharray="3 3" strokeOpacity={0.5} label={{ value: 'DEPLETION', fill: '#EF4444', fontSize: 9, position: 'right' }} />
