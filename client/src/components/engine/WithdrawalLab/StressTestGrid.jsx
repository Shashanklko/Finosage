            ...
            {data.map((s, i) => (
                <div key={i} className="stress-card">
                    <span className={`stress-card-tag ${s.tag}`}>
                        {s.tag === 'pass' ? '● GOOD' : s.tag === 'warn' ? '▲ CAUTION' : '✕ RISK'}
                    </span>
                    ...
                    <div style={{ display: 'flex', gap: '1.5rem', marginTop: '0.8rem' }}>
                        <div>
                            ...
                            <p style={{ fontSize: '0.45rem', color: '#4B5563', letterSpacing: '0.15em', textTransform: 'uppercase' }}>Likely Savings</p>
                        </div>
                        <div>
                            <p style={{ fontSize: '0.8rem', fontWeight: 300 }}>{s.depletionAge}</p>
                            <p style={{ fontSize: '0.45rem', color: '#4B5563', letterSpacing: '0.15em', textTransform: 'uppercase' }}>Money Runs Out Age</p>
                        </div>
                    </div>
                </div>
            ))}
