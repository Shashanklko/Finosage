    ...
    if (active && payload && payload.length) {
        return (
            <div className="engine-chart-tooltip">
                ...
                <p className="tip-gold">Most Likely: {formatINR(payload[0]?.value)}</p>
                {payload[2] && <p className="tip-blue">Good Market: {formatINR(payload[2]?.value)}</p>}
                {payload[1] && <p className="tip-red">Bad Market: {formatINR(payload[1]?.value)}</p>}
            </div>
        );
    }
    return null;
...
const ProjectionChart = ({ data, startAge = 60 }) => {
    ...
    return (
        ...
        >
            <h3 className="engine-chart-title">Expected Savings Growth Over Time</h3>
