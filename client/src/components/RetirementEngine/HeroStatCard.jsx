...
const HeroStatCard = ({ data }) => {
    if (!data) return null;
    const subMetrics = [
        { label: 'Corpus at Retirement', value: data.corpusAtRetirement ? formatINR(data.corpusAtRetirement) : '—' },
        { label: 'Median Corpus at End', value: formatINR(data.medianCorpus) },
        { label: 'Safe Withdrawal Rate', value: `${data.safeWithdrawalRate}%` },
    ];
        ...
        >
            ...
            <div className="hero-stat-layout">
                <div>
                    ...
                    <p className="hero-stat-label">Survival Probability</p>
                    <p className="hero-stat-desc">
                        Portfolio sustains through retirement in {Math.round(data.survivalRate * 100)} of 10,000 simulated scenarios.
                    </p>
                </div>
                <div>
                    <p className="hero-sparkline-label">SURVIVAL CURVE</p>
